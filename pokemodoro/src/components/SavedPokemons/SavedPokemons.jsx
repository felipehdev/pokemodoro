  ...
  const lPokemons = toPrint.map((pokeObj) => (
    <li className={S.liCtn} key={pokeObj.id}>
      ...
      <div className={S.spriteBg}>        
        ...
        <div className={S.spriteBg2}> </div>
      </div>
      <div className={S.pokeInfoCtn}>
        <span className={S.pokeName}>
          {pokeObj.name.charAt(0).toUpperCase() + pokeObj.name.slice(1)}
        </span>
        <div className={S.typeCtn}>
          <span className={`${S.pokeType} btn-${pokeObj.types[0].type.name}`}>
            {pokeObj.types[0].type.name.charAt(0).toUpperCase() +
              pokeObj.types[0].type.name.slice(1)}
          </span>
          {pokeObj.types[1] && (
            <span className={`${S.pokeType} btn-${pokeObj.types[1].type.name}`}>
              {pokeObj.types[1].type.name.charAt(0).toUpperCase() +
                pokeObj.types[1].type.name.slice(1)}
            </span>
          )}
        </div>
      </div>
